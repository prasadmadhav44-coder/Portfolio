...
function ProjectCard({ project, onClick, isCompact = false }) {
  const { title, shortDescription, tags, live, github } = project;
          ...
          >
            {live && (
              <motion.a
                ...
                whileTap={{ scale: 0.95 }}
                className={`flex items-center gap-2 font-semibold transition-colors group/link ${isCompact ? 'text-sm' : ''}`}
                style={{ color: 'var(--color-accent)' }}
              >
                <span>View Live</span>
                <ArrowRight className={`transition-transform group-hover/link:translate-x-1 ${isCompact ? 'w-3.5 h-3.5' : 'w-4 h-4'}`} />
              </motion.a>
            )}
            {github && (
              <motion.a
