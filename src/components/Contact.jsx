...
import { Phone, Mail as MailIcon, Github, Linkedin, Send, Loader2, CheckCircle2, XCircle } from 'lucide-react';
...
const PHONE_DISPLAY = '+91 6384360663';
const PHONE_WHATSAPP = '916384360663'; // wa.me needs country code, digits only, no + or spaces
const EMAIL_ADDRESS = 'prasadmadhav44@gmail.com';

const CONTACT_INFO = [
  { icon: Phone, label: 'Phone', value: PHONE_DISPLAY },
  { icon: MailIcon, label: 'Email', value: EMAIL_ADDRESS },
];

const SOCIALS = [
  { icon: Github, label: 'GitHub', href: 'https://github.com/prasadmadhav44-coder', external: true },
  { icon: Linkedin, label: 'LinkedIn', href: 'https://www.linkedin.com/in/madhavaprasadkg/', external: true },
  { icon: MailIcon, label: 'Email', href: `mailto:${EMAIL_ADDRESS}`, external: false },
  { icon: WhatsappIcon, label: 'WhatsApp', href: `https://wa.me/${PHONE_WHATSAPP}`, external: true },
];
...
function Contact() {
  ...
  const validateForm = () => {
    ...
    else if (formData.email.length > 100) newErrors.email = 'Email must be under 100 characters';

    if (!formData.subject.trim()) newErrors.subject = 'Subject is required';
    else if (formData.subject.length > 150) newErrors.subject = 'Subject must be under 150 characters';
  ...
  const handleSubmit = async (e) => {
    ...
    const sanitizedData = {
      ...
      company: formData.company, // honeypot — left untouched on purpose
    };
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setToast({ type: 'error', message: data.error || 'Could not send message. Try again later.' });
        return;
      }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
          ...
          >
            <div className="space-y-6">
              {/* Honeypot — invisible to real visitors, catnip for bots. */}
              <input
                type="text"
                name="company"
              ...
              <motion.button
                ...
                whileHover={{ scale: isLoading || justSent ? 1 : 1.02, y: isLoading || justSent ? 0 : -2 }}
                whileTap={{ scale: isLoading || justSent ? 1 : 0.98 }}
                disabled={isLoading}
                className={`w-full px-8 py-4 rounded-xl font-semibold text-lg transition-all duration-300 relative overflow-hidden ${
                  isLoading ? 'opacity-70 cursor-not-allowed' : ''
                }`}
                style={{
                  backgroundColor: justSent ? '#16a34a' : 'var(--color-accent)',
                  color: '#ffffff',
                }}
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            className="fixed bottom-24 right-4 left-4 sm:left-auto sm:right-8 md:bottom-8 px-6 py-4 rounded-2xl shadow-2xl z-50 flex items-center space-x-3"
            style={
              toast.type === 'success'
