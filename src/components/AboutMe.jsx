...
import { usePublicSingleton } from '../hooks/usePublicTable';
import { aboutFallback } from '../data/aboutData';

// Fixed icon rotation for quick facts — content (label/value) is
// CMS-editable, the icon per slot stays visually consistent.
const FACT_ICONS = [MapPin, GraduationCap, Sparkles];

function AboutMe() {
  const { data } = usePublicSingleton('about_content', { fallbackData: aboutFallback });
  const paragraphs = data?.paragraphs?.length ? data.paragraphs : aboutFallback.paragraphs;
  const rawFacts = data?.facts?.length ? data.facts : aboutFallback.facts;
  const facts = rawFacts.map((fact, idx) => ({ ...fact, icon: FACT_ICONS[idx % FACT_ICONS.length] }));
        ...
        <div className="flex flex-col lg:flex-row items-center gap-16">
          ...
          >
            <div className="relative">
              ...
              >
                <Logo className="h-28 w-28 md:h-32 md:w-32" color="var(--color-accent)" strokeWidth={20} />
              </div>
          ...
          >
            ...
            >
              {paragraphs.map((paragraph, idx) => (
                <p
                  key={idx}
                  className={idx === 0 ? 'text-xl md:text-2xl font-light' : 'text-base md:text-lg'}
                  style={idx === 0 ? { color: 'var(--color-text)' } : undefined}
                >
                  {paragraph}
                </p>
              ))}
